...
const props = defineProps(["product", "flavors"]);
const searchText = ref("");
const showResults = ref(false);
const listFlavors = ref(props.flavors);
const selectedFlavors = ref([]);
const showAddButton = ref(false);
const { addFlavorToTrolley, isFlavorInTrolley } = useUserTrolleyStore();
const { trans } = useTranslationsStore();
const { showFooter, hiddeFooter } = useFooterHelper();

onMounted(() => {
  if (!props.product.multiple_flavors && props.flavors.length) {
    let flavor = props.flavors[0];
    if (!isFlavorInTrolley(flavor)) {
      selectFlavorHandle(flavor);
    }
  }
});

const filteredList = () => {
  return listFlavors.value.filter(
    (flavor) =>
...
<template>
  <div v-if="product.multiple_flavors" class="select-2_container">
    <div
      v-if="selectedFlavors.length"
      id="select-2_container_selected_container"
      class="select-2_container_selected_container"
    >
      <div id="flavor-titles" class="flavor-titles">
        {{ trans("29f7e87f-5967-4b6f-832a-feaefc3adcdf") }} ({{
          selectedFlavors.length
        }}):
      </div>
      <div
        v-for="flavor in selectedFlavors"
        class="select-2_container_selected_container_item"
      >
        <div>
          {{ flavor.name }}
        </div>
        <div
          class="select-2_container_selected_container_item_remove"
    ...
    <div class="flavor-titles">
      {{ listFlavors.length }}
      {{ trans("075e395e-9cd8-4959-bce6-e6cab354e4af") }}:
    </div>
    <div class="select-2_container_input_container" id="select-2">
      <input
        v-model="searchText"
        class="form-control select-2_container_input shadow"
        type="text"
        :placeholder="trans('2e69d193-03a4-4d90-9867-5b3ac90d8caf')"
        @focus="focusInputHandle"
        @blur="blurInputHandle"
      />
    </div>
    <div
      class="select-2_container_result_container scroll-custom overflow-auto shadow"
      id="select-2_container_result_container"
    >
      <div v-if="listFlavors.length">
        <div
          v-for="flavor in filteredList()"
          class="select-2_container_result_container_item"
      ...
      <div
        v-else
        class="not-found-flavors-container d-flex justify-content-center"
      >
  ...
  <div
    v-if="showAddButton"
    class="product-detail-flavors_add_button"
    @click="addFlavorsToTrolley"
  >
